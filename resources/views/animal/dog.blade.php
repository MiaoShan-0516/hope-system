...
<div class="container flex flex-col items-center">
	<span class="text-xl font-bold mb-3">狗狗资料</span>
    ...
    <table class="w-full">
      ...
      <tbody>
        <tr class="border-solid border-2">
          <td class="p-1 border-solid border-2">{{ $dog->name }}</td>
          <td class="p-1 border-solid border-2">{{ date('m-d', strtotime($dog->vacination_date)) }}</td>
          <td class="p-1 border-solid border-2">{{ $dog->area->name }}</td>
          <td class="p-1 border-solid border-2">{{ $dog->take_care_by ?? '-' }}</td>
          <td>
            <a href="{{ route('animal.info', $dog) }}">
