...
<div class="container flex flex-col items-center">
	<span class="text-xl font-bold mb-3">其他动物资料</span>
    ...
    <table class="w-full">
      ...
      <tbody>
        <tr class="border-solid border-2">
          <td class="p-1 border-solid border-2">{{ $other->name }}</td>
          <td class="p-1 border-solid border-2">{{ date('m-d', strtotime($other->vacination_date)) }}</td>
          <td class="p-1 border-solid border-2">{{ $other->area->name }}</td>
          <td class="p-1 border-solid border-2">{{ $other->take_care_by ?? '-' }}</td>
          <td>
            <a href="{{ route('animal.info', $other) }}">
